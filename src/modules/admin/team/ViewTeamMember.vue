    ...
    <div class="lists">
        ...
        <div class="table_responsive">
            <table class="table">
                ...
                <tbody>
                  <tr v-for="(data, index) in get_team_member" :key="data._id">
                    <th scope="row" class="table_td">{{ index+1 }}</th>
                    <td class="table_td"><img :src="data.feature_image" style="width:50px; height:50px;" alt="" ></td>
                    <td class="table_td">{{data.full_name}}</td>
                    <td class="table_td">{{data.designation}}</td>
                    <td class="table_td">{{data.short_bio}}</td>
...
export default{
    setup() {
        ...
        const getTeamMember = async()=>{
            const data = await projectApi.viewTeamMemeberData(page.value, route.params.pid, view_limit);
            console.log(data.data);
            total_result.value = data.data.results;
            get_team_member.value =data.data.data;
